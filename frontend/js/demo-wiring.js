(function () {
  ...
  async function motherDashboard() {
    try {
      ...
      if (start) start.addEventListener('click', () => { window.location.href = '/pages/mother/check-in.html'; });
  ...
  async function checkIn() {
    ...
    continueButton.addEventListener('click', async () => {
      ...
      try {
        ...
        window.location.href = '/pages/mother/result.html';
      } catch (error) {
  ...
  function authPage() {
    ...
    loginForm?.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        window.location.href = routes[result.user?.role || result.data?.role || 'mother'];
    ...
    registerForm?.addEventListener('submit', async (event) => {
      ...
      try {
        const result = await api.register({
          ...
          phone: form.get('phone'), role: form.get('role'), language: 'English',
          deliveryDate: form.get('deliveryDate'), village: form.get('village')
        });
        window.location.href = routes[result.user?.role || result.data?.role || 'mother'];
      } catch (error) { showMessage(error.message); }
    });
  ...
  function init() {
    ...
    if (path.includes('/pages/auth/')) return authPage();
  }
